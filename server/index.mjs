...
const movieService=new MovieService({token:process.env.TMDB_READ_TOKEN,store});
const pairingService=new PairingService({movies:movieService,store,apiKey:process.env.OPENAI_API_KEY,model:process.env.OPENAI_MODEL||'gpt-4.1-mini',dailyLimit:Number(process.env.MAX_DAILY_GENERATIONS||100)});
if(!Number.isInteger(pairingService.dailyLimit)||pairingService.dailyLimit<1)throw new Error('MAX_DAILY_GENERATIONS must be a positive integer.');
const limits=new Map(),salt=randomBytes(32).toString('hex');
function rate(req,kind,max,windowMs){
 ...
 const address=process.env.TRUST_PROXY==='1'?String(req.headers['x-forwarded-for']||req.socket.remoteAddress).split(',')[0].trim():req.socket.remoteAddress;
 const id=createHash('sha256').update(`${salt}:${address}:${kind}`).digest('hex');let item=limits.get(id);
 if(!item){if(limits.size>=10000)throw new PublicError(429,'Too many requests. Please try again later.');item={count:0,until:now+windowMs};limits.set(id,item);}
 if(++item.count>max)throw new PublicError(429,'Too many requests. Please wait a moment and try again.');
}
function json(res,status,data){res.writeHead(status,{'Content-Type':'application/json; charset=utf-8','Cache-Control':'no-store'});res.end(JSON.stringify(data));}
async function body(req){
 if(!String(req.headers['content-type']||'').startsWith('application/json'))throw new PublicError(415,'Use a JSON request.');
 if(Number(req.headers['content-length']||0)>4096)throw new PublicError(413,'The selection is too large.');
 let length=0,chunks=[];for await(const chunk of req){length+=chunk.length;if(length>4096)throw new PublicError(413,'The selection is too large.');chunks.push(chunk);}
...
// const url=new URL(req.url,'http://local.invalid');
 if(url.pathname.startsWith('/api/')){
  ...
  if(req.method==='POST'&&url.pathname==='/api/pairings'){
   if(!origin)throw new PublicError(403,'A permitted website origin is required.');rate(req,'pairings',10,600000);const input=await body(req);json(res,200,await pairingService.generate(input));return;
  }
  throw new PublicError(404,'Movie service route not found.');
 }
 if(!['GET','HEAD'].includes(req.method))throw new PublicError(405,'Method not allowed.');
 let pathname;try{pathname=decodeURIComponent(url.pathname);}catch{throw new PublicError(400,'Invalid path.');}
 const file=resolve(root,`.${pathname==='/'?'/index.html':pathname}`);
 if(!file.startsWith(root.endsWith(sep)?root:root+sep))throw new PublicError(403,'Access denied.');
